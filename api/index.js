...
// ─── CORS helper ──────────────────────────────────────────────────────────────
// The site and the API share the same origin, so cross-origin access is only
// allowed for origins whose host matches the request host (e.g. local dev on a
// different port) or hosts explicitly listed in ALLOWED_ORIGINS. Reflecting
// arbitrary origins with credentials would let any site read authed responses.
function setCors(req, res) {
  ...
  try {
    ...
    allowed = originHost === reqHost || originHost === 'localhost' || originHost === '127.0.0.1' || extra.includes(origin);
  } catch (e) { /* malformed origin header — no CORS headers */ }
  if (!allowed) return;
...
async function createPurchase(req, res) {
  ...
  if (await isEnrolled(user.id, course_id)) {
    return res.status(400).json({ success: false, error: 'You already have access to this course' });
  }

  // Block duplicate submissions while a request is still pending
  const dup = await query(
    `SELECT id FROM purchases WHERE user_id=$1 AND course_id=$2 AND status='pending'`,
    [user.id, course_id]
  );
  if (dup.rows.length) {
...
async function adminPurchases(req, res) {
  ...
  if (req.method === 'PUT') {
    ...
    if (!validStatuses.includes(status)) return res.status(400).json({ success: false, error: 'Invalid status. Use: pending, approved, or rejected' });

    const p = await query('SELECT user_id, course_id, status FROM purchases WHERE id = $1', [id]);
    if (!p.rows.length) return res.status(404).json({ success: false, error: 'Purchase not found' });
    const prev = p.rows[0];
    ...
    if (status === 'approved') {
      ...
      `, [prev.user_id, prev.course_id, admin.id]);
    }

    // If a previously approved purchase is rejected, revoke the access it granted
    if (status === 'rejected' && prev.status === 'approved') {
      await query('DELETE FROM user_courses WHERE user_id=$1 AND course_id=$2', [prev.user_id, prev.course_id]);
    }
...
async function authRegister(req, res) {
  ...
  // No verification email is sent in the current flow — don't tell users to wait for one.
  return res.status(201).json({ success: true, message: 'Registration successful. You can now log in.', user: r.rows[0] });
}
...
async function authLogout(req, res) {
  ...
  const secureFlag = (req.headers['x-forwarded-proto'] === 'https') ? '; Secure' : '';
  res.setHeader('Set-Cookie', `session_token=; HttpOnly; Path=/; Max-Age=0; SameSite=Strict${secureFlag}`);
...
// Bootstraps the schema and the first admin account. Once an admin exists this
// endpoint requires a logged-in admin session — otherwise anyone could reset
...
async function adminInit(req, res) {
  ...
  try {
    const chk = await pool.query('SELECT 1 FROM users WHERE is_admin=TRUE LIMIT 1');
    adminExists = chk.rows.length > 0;
...
// ADD THIS BLOCK RIGHT HERE:
  ...
  const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'asikrac@gmail.com';
  const ADMIN_PASS  = process.env.ADMIN_PASSWORD || 'asikasik';
  ...
  return res.status(200).json({ success: true, message: `Setup complete. Admin account: ${ADMIN_EMAIL}` });
}
...
async function userUpdateProfile(req, res) {
  ...
  if (type === 'profile') {
    ...
    if (cleanName.length > 50)
      return res.status(400).json({ success: false, error: 'Display name must be 50 characters or fewer' });
    ...
    const nameDup = await query('SELECT id FROM users WHERE LOWER(username)=LOWER($1) AND id!=$2', [cleanName, user.id]);
    if (nameDup.rows.length) return res.status(400).json({ success: false, error: 'That display name is already taken' });
    await query('UPDATE users SET username=$1, email=$2 WHERE id=$3', [cleanName, newEmail.toLowerCase().trim(), user.id]);
...
async function adminSetRole(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'POST only' });
  // Only admin/manager may change roles — task_manager must not be able to
  // promote other accounts (privilege escalation).
  const staff = await requireManagerOrAbove(req, res); if (!staff) return;
